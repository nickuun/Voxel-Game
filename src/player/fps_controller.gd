...
func _try_place_block() -> void:
	...
	if BlockDB.is_notch(id_to_place):
		# Use the hit position you already compute; we want sub inside the target cell.
		if hit.is_empty(): return
		# Just push a tiny epsilon into the target cell along the face normal (like blocks)
		var wpos = hit.position + hit.normal * 0.5 + hit.normal * EPS
		world.place_notch_at_world(wpos, id_to_place)
